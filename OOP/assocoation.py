# Ассоциация - означает, что внутри одного объекта будет существовать другой объект в качестве атрибута
# 1. Композиция - сильная связь
# 2. Агрегация - слабая связь


'Композиция - это когда стена не существует отдельно от комнаты. Она создаётся при создании комнаты и полностью управляется классом комнаты'
# class Wall:
...
'Агрегация - это когда экземпляр двигателя создаётся где-то в другом месте и передаётся в класс Авто в качестве параметра'
# class Engine:
#     country = 'Germany'
...
#         'когда мы создаём объект внутри - композиция'
...
# del nokia
# 'при удалении нокии батарейка остаётся'
